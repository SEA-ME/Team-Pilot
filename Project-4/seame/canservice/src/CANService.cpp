...
int main(){
  ...
  std::shared_ptr<SEAMEStubImpl> moonService = std::make_shared<SEAMEStubImpl>();;
  
  while (!runtime->registerService("local", "seame", moonService)) {
    ...
    moonService->disPublisher(frame.data[5]);
