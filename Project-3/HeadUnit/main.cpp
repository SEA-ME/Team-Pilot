...
int main(int argc, char *argv[])
{
    ...
    std::shared_ptr<HeadUnitProxy<>> moonProxy = runtime->buildProxy<HeadUnitProxy>("local", "headunit");
    ...
    std::shared_ptr<class QTimer> sendTimer;
    sendTimer = std::make_shared<QTimer>();
    QObject::connect(sendTimer.get(), SIGNAL(timeout()), &ambient, SLOT(OnTimerCallbackFunc()));
/*
    while (true)
    {
        // CAN
